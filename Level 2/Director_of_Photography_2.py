...
## Explanation: Because distances are all based on the location of the actor, we
## will find the number of artistic photos that can be taken at each actor
## position. Given an actor position, the total number of artistic photos that
## can be taken can be calculated in O(1) time by multipling the number of valid
## photographer and background positions. To efficiently get the number of
## photographer positions in an interval, create a list T where T{i} stores the
## number of photographer positions before index i. Using this list, we can
## calculate the number of photographers in a interval with
## T[end + 1] - T[start].
